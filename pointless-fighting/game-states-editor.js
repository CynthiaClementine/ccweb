...
class State_Edit_Collision extends State_Edit {
	...
	doWorld() {
		//don't do the whole world thing if a panel is up
		if (this.panels.reduce((a, b) => a + b.a, 0) > 0) {
			return;
		}
		ctx.fillStyle = color_background;
		ctx.fillRect(0, 0, canvas.width, canvas.height);
	...
	syncNameButton() {
		...
		this.panels[0].button.x = this.buttons[0].x - (this.buttons[0].w / 2) - 0.08;
	}
	...
	handleMouseDown() {
		...
		if (this.panels[0].panelUp) {
			...
			if (this.panels[0].value != undefined) {
				loading_map = getZone(this.panels[0].value);
				this.exit = undefined;
...
class State_Edit_Entities extends State_Edit {
	constructor() {
		...
		this.entityPanel = new UI_Panel(this.sbarW / 3, 0.08, 0.03, 0.04, "+", false, false, 0.8, 0.6, ["Spike", "Portal_Round", "Portal_Polygon"]);
