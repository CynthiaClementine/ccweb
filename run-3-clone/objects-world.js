...
class PushableBox extends Tile_Box_Ringed {
	constructor(x, y, z, parent, size, rot) {
		super(x, y, z, size, [(Math.PI * 2.5) - parent.theta, rot], parent);
	...
	chooseParent() {
		...
		if (tunnelZ > (ref.len * ref.tileSize) + tunnel_transitionLength || tunnelZ < 0) {
			...
			ref.freeObjs.splice(ref.freeObjs.indexOf(this), 1);
			this.parent.freeObjs.push(this);
			this.normal[0] = (Math.PI * 2.5) - this.parent.theta;

			//decrease dz based on normal change
			this.dz *= (Math.cos(ref.theta - this.parent.theta) + 0.3) / 1.3;
		}
	}
	...
	tick() {
		...
		this.houseState = this.houseState ?? loading_state;
		...
		if (this.playerDist < this.size * 2) {
			...
			var relPos = spaceToRelativeRotless([player.x, player.y, player.z], [this.x, this.y, this.z], [this.normal[0], this.normal[1]]);
			//if on the correct side and within range, push box
			if (Math.abs(relPos[0]) < (this.size / 2) + player.r && Math.abs(relPos[1]) < (this.size / 2) + player.r && Math.abs(relPos[1]) < (this.size / 2) + player.r) {
				//pushy stuff for each side
				if (Math.abs(relPos[0]) > (this.size - player.r) / 2) {
					...
					//also cap dz
					this.dz = clamp(this.dz, -9 * physics_boxMultiplier, 9 * physics_boxMultiplier);
					
				}

				if (Math.abs(relPos[2]) > (this.size - player.r) / 2) {
					//different push (large vs small based on how far out player is)
					this.dx = boolToSigned(relPos[2] < 0) * (physics_boxSidePush + (physics_boxSidePush * (Math.abs(relPos[2]) > this.size)));
					this.dz *= 0.95 + (0.025 * (Math.abs(relPos[2]) < this.size));
				}

				if (Math.abs(relPos[1]) > (this.size - player.r) / 2) {
					this.dy = boolToSigned(relPos[1] < 0) * (physics_boxSidePush + (physics_boxSidePush * (Math.abs(relPos[1]) > this.size)));
					this.dz *= 0.95 + (0.025 * (Math.abs(relPos[1]) < this.size));
				}
				deathCount = 0;
			}
			//regular'ol good'ol box collisin
			this.collideWithEntity(player);
		}

		//if outside bounds of the tunnel, don't be
		if (this.dx != 0 || this.dy != 0 || this.dz != 0) {
			this.parent.r -= (this.size * 0.7);
			if (!this.parent.coordinateIsInTunnel(this.x, this.y, this.z, false)) {
				var relPos = spaceToRelativeRotless([this.parent.x, this.parent.y, this.parent.z], [this.x, this.y, this.z], this.normal);
		...
		if (this.dx != 0 || this.dy != 0 || this.dz != 0) {
			var worldDX = polToCart(this.normal[0], this.normal[1], this.dx);
			var worldDY = polToCart(this.normal[0], this.normal[1] + (Math.PI / 2), this.dy);
			var worldDZ = polToCart(this.normal[0] - (Math.PI / 2), 0, this.dz);
	...
	beDrawn() {
		...
		if (editor_active) {
			drawCrosshair([this.x, this.y, this.z], this.normal, [this.normal[0], this.normal[1] + (Math.PI / 2)], [this.parent.theta, 0]);
		}
	}
}
...
class Star_Special extends Star {
	...
	beDrawn_normally(screenX, screenY) {
		...
		ctx.filter = `blur(${Math.floor(this.drawR / 4)}px)`;
		drawCircle(this.color, screenX, screenY, this.drawR);
		ctx.filter = `none`;
		ctx.globalAlpha = render_starOpacity ** 0.5;
		drawCircle(this.color, screenX, screenY, this.drawR * 0.6);
