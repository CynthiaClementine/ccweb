...
function determineTutorialText() {
	//if done all the tutorial things don't try again
	if (tutorial.hasDone.reduce((a, b) => {return a && b}) == true) {
		return;
	}

	//possibly complete tutorial tasks
	if (game_mode == 0) {
	...
	//if entered a level that's been completed show escape tet
	if (tutorial.hasDone[1] && game_map.completed && !tutorial.hasDone[3]) {
		text_buffer = tutorial.texts[3];
		text_time = text_time_static;
	}
}
...
function updateSettings() {
	...
	tutorial.hasDone[3] = data_persistent.tut;
