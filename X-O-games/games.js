...
class Game {
	constructor() {
		this.dims = [1, 1, 1];
		this.pieces = "XO";
		this.players = [this.pieces[0], this.pieces[1]];
		this.thinkDist = -1;
		this.utilGoals = {
			"X": 1,
			"O": -1
	...
	utilityStandard(boardState, width, height, requiredInARow) {
		...
		for (var c=0; c<boardState.length; c++) {
			valuesFromBoard[c] = +(boardState[c] == this.pieces[0]) - +(boardState[c] == this.pieces[1])
		}
...
class OrderAndChaos extends Game {
	constructor() {
		super();
		this.dims = [6, 6, 5];
		this.pieces = "xo"
		this.players = ["order", "chaos"];
	}

	utilityFor(board) {
		return Math.abs(super.utilityFor(board));
	}
	...
	constructChildren(boardState, player) {
		...
		for (var y=0; y<sz[1]; y++) {
			for (var x=0; x<sz[0]; x++) {
				if (boardState[(y * sz[1]) + x] == boardEmpty) {
					arr[x + (sz[1] * y)] = [boardChars[x] + boardChars[y], this.applyMove(boardState, x + (sz[1] * y), this.pieces[0])];
					arr[x + (sz[1] * y) + (sz[0] * sz[1])] = [boardChars[x] + boardChars[y], this.applyMove(boardState, x + (sz[1] * y), this.pieces[1])];
				}
			}
		}
		return arr;
		...
		for (var c=1; c<9; c++) {
			for (var r=1; r<9; r++) {
				...
				if (boardState[nowIndex] == boardEmpty) {
					...
					if (this.childIsGood(boardState, player, opposite, nowIndex)) {
...
var games = {
	...
	"order-and-chaos": new OrderAndChaos(),
	"othello": new Othello(),
}
...
var games_desc = {
	...
	"order-and-chaos": [`Played on a 6 by 6 board.`, `One player is order, and the other is chaos.`, `Both players can place an X or O.`, `Order wins by making at least 5 in a row, and chaos wins by preventing that.`, `Order plays first. To play type either X or O and then the number of the space you want to play at.`],
